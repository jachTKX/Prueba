...
class news_noticias_editar_publicadas_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['news_noticias_editar_publicadas']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['news_noticias_editar_publicadas']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['news_noticias_editar_publicadas']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['news_noticias_editar_publicadas']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['news_noticias_editar_publicadas']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['news_noticias_editar_publicadas']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['news_noticias_editar_publicadas']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['news_noticias_editar_publicadas']['contr_total_geral'] = "OK";
   } 
   ...
   //----- 
   function Calc_resumo_groupby_1($destino_resumo)
   {
      global $nm_lang, $altera_where_data;
      $this->nm_data = new nm_data("en_us");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['news_noticias_editar_publicadas']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['news_noticias_editar_publicadas']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('news_categorias_categoria_id' => "news_categorias.categoria_id");
      $cmps_quebra_atual = array("news_categorias_categoria_id");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_groupby_1($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['news_noticias_editar_publicadas']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['news_noticias_editar_publicadas']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_groupby_1($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('news_noticias_noticia_id' => 'N','news_categorias_categoria_id' => 'N','news_noticias_reporter_id' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['news_noticias_editar_publicadas']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['news_noticias_editar_publicadas']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['news_noticias_editar_publicadas']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['news_noticias_editar_publicadas']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['news_categorias_categoria_id']['reg'] = "S";
      $format_dimensions['news_categorias_categoria_id']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "news_categorias_categoria_id") {
                  $this->Lookup->lookup_groupby_1_news_categorias_categoria_id($$Cada_dim);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
