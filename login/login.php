...
//
   ...
   $_SESSION['scriptcase']['login']['glo_nm_path_cache']  = "C:/Program Files/NetMake/v9-php73/wwwroot/scriptcase/file/cache";
...
//
   function init()
   {
      ...
      $this->nm_nome_apl     = "Login application of the News system"; 
      $this->nm_seguranca    = ""; 
      $this->nm_grupo        = "news"; 
...
<?php
      ...
      $this->link_menu = $this->sc_protocolo . $this->server . $this->path_link . "" . SC_dir_app_name('menu') . "/menu.php";
...
// 
  function conectDB()
  {
      ...
      { 
          $this->Db = db_conect_devel($_SESSION['scriptcase']['login']['glo_nm_conexao'], $this->root . $this->path_prod, 'news', 1, $this->force_db_utf8); 
      } 
      else 
      { 
  ...
  function setConnectionHash() {
    if (isset($_SESSION['scriptcase']['nm_sc_retorno']) && !empty($_SESSION['scriptcase']['nm_sc_retorno']) && isset($_SESSION['scriptcase']['login']['glo_nm_conexao']) && !empty($_SESSION['scriptcase']['login']['glo_nm_conexao'])) {
      list($connectionDbms, $connectionHost, $connectionUser, $connectionPassword, $connectionDatabase) = db_conect_devel($_SESSION['scriptcase']['login']['glo_nm_conexao'], $this->root . $this->path_prod, 'news', 1, $this->force_db_utf8);
    }
    else {
...
//
    ...
    if ($Sem_Session && (!isset($nmgp_start) || $nmgp_start != "SC")) {
        ...
        elseif (is_file($root . $_SESSION['scriptcase']['login']['glo_nm_path_imag_temp'] . "/sc_apl_default_news.txt")) {
            $apl_def = explode(",", file_get_contents($root . $_SESSION['scriptcase']['login']['glo_nm_path_imag_temp'] . "/sc_apl_default_news.txt"));
        }
        if (isset($apl_def)) {
            if ($apl_def[0] != "login") {
            ...
            if (isset($_COOKIE['sc_actual_lang_news'])) {
                $_SESSION['scriptcase']['login']['session_timeout']['lang'] = $_COOKIE['sc_actual_lang_news'];
            }
        }
    }
    ...
    if (isset($_POST['rs']) && !is_array($_POST['rs']) && 'ajax_' == substr($_POST['rs'], 0, 5) && isset($_POST['rsargs']) && !empty($_POST['rsargs']) && !isset($_SESSION['scriptcase']['login']['session_timeout']['redir']))
    {
        ...
        if ('ajax_login_validate_senha' == $_POST['rs'])
        {
    ...
    { 
        ...
        if (isset($glo_nome_usuario)) 
        {
            $_SESSION['glo_nome_usuario'] = $glo_nome_usuario;
        }
    } 
    ...
    else
    {
        ...
        if (isset($_POST["glo_login"])) 
        {
            ...
            nm_limpa_str_login($_SESSION['glo_login']);
        }
        ...
        if (isset($_POST["glo_nome_usuario"])) 
        {
            $_SESSION['glo_nome_usuario'] = $_POST["glo_nome_usuario"];
            nm_limpa_str_login($_SESSION['glo_nome_usuario']);
        }
        if (isset($_GET["glo_nome_usuario"])) 
        {
            $_SESSION['glo_nome_usuario'] = $_GET["glo_nome_usuario"];
            nm_limpa_str_login($_SESSION['glo_nome_usuario']);
        }
    ...
    sajax_export("ajax_login_validate_senha");
...
//
    ...
    function ajax_login_validate_senha($senha, $script_case_init)
    {
        global $inicial_login;
        //register_shutdown_function("login_pack_ajax_response");
        $inicial_login->contr_login->NM_ajax_flag          = true;
        $inicial_login->contr_login->NM_ajax_opcao         = 'validate_senha';
        $inicial_login->contr_login->NM_ajax_info['param'] = array(
    ...
    function ajax_login_submit_form($login, $senha, $nm_form_submit, $nmgp_url_saida, $nmgp_opcao, $nmgp_ancora, $nmgp_num_form, $nmgp_parms, $script_case_init, $csrf_token, $bok)
    {
        global $inicial_login;
        //register_shutdown_function("login_pack_ajax_response");
        $inicial_login->contr_login->NM_ajax_flag          = true;
