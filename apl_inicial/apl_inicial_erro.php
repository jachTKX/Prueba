...
$old_error_handler = set_error_handler("apl_inicial_scriptcase_error_handler");

function apl_inicial_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['apl_inicial']['contr_erro']) && $_SESSION['scriptcase']['apl_inicial']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_apl_inicial->contr_apl_inicial->Apl_com_erro = true;
            if ($inicial_apl_inicial->contr_apl_inicial->NM_ajax_flag)
            {
                $inicial_apl_inicial->contr_apl_inicial->NM_ajax_info['errList']['geral_apl_inicial'][] = $msg;
                return;
            }
            apl_inicial_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function apl_inicial_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['apl_inicial'])  && '' != $_SESSION['scriptcase']['error_icon']['apl_inicial'])  ? $_SESSION['scriptcase']['error_icon']['apl_inicial']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['apl_inicial']) && '' != $_SESSION['scriptcase']['error_close']['apl_inicial']) ? $_SESSION['scriptcase']['error_close']['apl_inicial'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['apl_inicial']) && '' != $_SESSION['scriptcase']['error_span_title']['apl_inicial']) ? $_SESSION['scriptcase']['error_span_title']['apl_inicial'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['apl_inicial']) && '' != $_SESSION['scriptcase']['error_icon_title']['apl_inicial']) ? $_SESSION['scriptcase']['error_icon_title']['apl_inicial'] : '';
?>
