...
$old_error_handler = set_error_handler("apl_inicial_mob_scriptcase_error_handler");

function apl_inicial_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['apl_inicial_mob']['contr_erro']) && $_SESSION['scriptcase']['apl_inicial_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_apl_inicial_mob->contr_apl_inicial_mob->Apl_com_erro = true;
            if ($inicial_apl_inicial_mob->contr_apl_inicial_mob->NM_ajax_flag)
            {
                $inicial_apl_inicial_mob->contr_apl_inicial_mob->NM_ajax_info['errList']['geral_apl_inicial_mob'][] = $msg;
                return;
            }
            apl_inicial_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function apl_inicial_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['apl_inicial_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['apl_inicial_mob'])  ? $_SESSION['scriptcase']['error_icon']['apl_inicial_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['apl_inicial_mob']) && '' != $_SESSION['scriptcase']['error_close']['apl_inicial_mob']) ? $_SESSION['scriptcase']['error_close']['apl_inicial_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['apl_inicial_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['apl_inicial_mob']) ? $_SESSION['scriptcase']['error_span_title']['apl_inicial_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['apl_inicial_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['apl_inicial_mob']) ? $_SESSION['scriptcase']['error_icon_title']['apl_inicial_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['apl_inicial']['error_buffer'] .= ob_get_flush();
}
